...
func createGoModFile(op errors.Op, mod, dir string) error {
	...
	// Add go statement. This is how 'go mod init' determines the go version to use.
	tags := build.Default.ReleaseTags
	version := tags[len(tags)-1]
	if !strings.HasPrefix(version, "go") || !modfile.GoVersionRE.MatchString(version[2:]) {
		return errors.New(errors.Go, fmt.Sprintf("unrecognized default go version %q", version), op)
	}
...
func (realGo) Build(ctx context.Context, pkg, outPath, dir string) error {
	return execGo(ctx, errors.Op("Go.Build"), dir, "build", "-o", outPath, pkg)
}

func (realGo) GetD(ctx context.Context, mod, dir string) error {
	return execGo(ctx, errors.Op("Go.GetD"), dir, "get", "-d", mod)
}
...
func (realGo) ListU(ctx context.Context, mod, dir string) (GoModule, error) {
	...
	if err := cmd.Run(); err != nil {
		...
		return gm, errors.New(errors.Go, msg, op, err)
	}
...
func execGo(ctx context.Context, op errors.Op, dir string, args ...string) error {
	cmd := exec.CommandContext(ctx, "go", args...)
	cmd.Dir = dir
